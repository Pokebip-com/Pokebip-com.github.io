...
async function getData() {
    const [
        ...
        monsterNameResponse,
        moveNameResponse,
        passiveSkillMoveNumberResponse,
    ...
    passiveSkillMoveNumber = await passiveSkillMoveNumberResponse.json();
...
function getMovePassiveDigitTags(id) {
    ...
    for(let i = 1; i <= 20; i +=2) {
        switch(movePassiveDigit[id][`param${i}`]) {
            case "1":
                if(movePassiveDigit[id][`param${i+1}`] === "")
                    continue;

                if(typeof tags["Digit"] === "undefined") {
                    tags["Digit"] = {};
                }
                const digit = passiveSkillMoveNumber[movePassiveDigit[id][`param${i+1}`]];

...
function setMoveNames() {
	Object.keys(moveNames).forEach( id => {
		movesList[id] = movesList[id] || {};
		movesList[id].name = moveNames[id];
	});
}

function setMoveDescriptions() {
	let idx = null;
	let descr = "";
	const idTagReplace = '[Name:MoveDescriptionPartsIdTag Idx=\"{ID}\" ]';
	const regExBase = /\[Name:MoveDescriptionPartsIdTag Idx="(\w+)" ]/gi;
	let re = regExBase;

	Object.keys(moveDescription).forEach( id => {
		movesList[id] = movesList[id] || {};

		idx = re.exec(moveDescription[id]);

		if(idx != null) {
			descr = moveDescription[id];

			do {
				descr = descr.replace(idTagReplace.replace("{ID}", idx[1]), moveDescriptionParts[idx[1]].replace("]]", "]"));
				re.lastIndex = 0;
			} while((idx = re.exec(descr)) !== null);

			if(id == 10314) {
				movesList[id].description = getSkillMoveDescr2(descr, id);
			}
			else
				movesList[id].description = getSkillMoveDescr2(descr, id);
		}
		else {
			movesList[id].description = moveDescription[id];
		}
	});
}
...
function setPassiveDescriptions() {
	...
	Object.keys(passiveSkillDescription).forEach( id => {
		...
		if(idx != null) {
			...
			if(id == 18107901) {
				passiveList[id].description = getSkillMoveDescr2(descr, id);
			}
			else {
				passiveList[id].description = getSkillMoveDescr(descr, id);
			}
		}
		else {
...
function setPassives(id) {
	passivesTable.innerHTML = "";
	passivesTable.innerHTML += "<thead><tr><th colspan='2'>Talents passifs</th></tr><tr><th>Nom</th><th>Effet</th></tr></thead>\n";

	for(let i = 1; i < 5; i++) {
		let passiveId = trainerInfos[id][`passive${i}Id`];

		if(passiveId === 0)
			continue;

		passivesTable.innerHTML += `<tr><td>${passiveList[passiveId].name}</td><td>${passiveList[passiveId].description}</td></tr>`;
	}
}

function setMoves(id) {
	movesTable.innerHTML = "";
	movesTable.innerHTML += "<thead><tr><th colspan='10'>Capacités</th></tr>";
	movesTable.innerHTML += "<tr><th>Capa.</th><th>Nom</th><th>Type</th><th>Catég.</th><th>Puis.</th><th>Précis.</th><th>Jauge</th><th>Cible</th><th>Effet</th><th>Limite</th></tr></thead>\n";

	console.log(trainerInfos[id]);

	for(let i = 1; i < 5; i++) {
		let moveId = trainerInfos[id][`move${i}Id`];

		if(moveId === 0)
			continue;

		console.log(moveInfos[moveId]);

		movesTable.innerHTML += `<tr><td></td><td>${movesList[moveId].name}</td><td>${movesList[moveId].description}</td></tr>`;
	}
}
...
function getMovePassiveDigitTags(id) {
	...
	//*
	if(id == 430 || id == 86 || id == 204 || id == 10314)
		console.log(movePassiveDigit[id]);
	//*/
...
function getMovePassiveDigitTags2(id) {
	tags = {};
	...
	for(let i = 1; i <= 20; i +=2) {
		switch(movePassiveDigit[id][`param${i}`]) {
			...
			case "2":
				tags[Object.keys(tags).length + 1] = {
					"value": tagNames[movePassiveDigit[id][`param${i+1}`]],
					"type": "Name",
					"subtype": "ReferencedMessageTag",
				};
				break;
		}
	}
...
function getNextTagsUsedValue(tagsUsed, tags) {

	if(tagsUsed < Object.keys(tags).length) {
		return (tagsUsed+1);
	}
...
function getSkillMoveDescr2(descr, id) {
	let tags = getMovePassiveDigitTags2(id);
	...
	while(descr.indexOf("[") !== -1 && breaker < 15) {
		...
		const idxValue = getArgumentValue(substring, "Idx");
		const refValue = getArgumentValue(substring, "Ref");

		const singularValue = getArgumentValue(substring, "S");
		const pluralValue = getArgumentValue(substring, "P");

		if(tagsLength > 0 && tagsUsed >= tagsLength && (tagValue == "ReferencedMessageTag" || tagType == "Digit")) {
			console.log(tagsLength);
			console.error("TAGS USED PROBLEM");
			tagsUsed--;
		}
		...
		if(idxValue != "") {
			switch(tagValue) {
				case "MoveId":
					Idx[idxValue] = moveNames[idxValue].replace("\n", " ");
					break;
				case "PassiveSkillId":
					Idx[idxValue] = passiveList[idxValue].name.replace("\n", " ");
					break;
				default:
					Idx[idxValue] = tags[getNextTagsUsedValue(tagsUsed, tags)].value;
			}
		}
		...
		switch(tagType) {
			case "Name":
				switch(tagValue) {
					case "ReferencedMessageTag":
						tagsUsed = getNextTagsUsedValue(tagsUsed, tags);
						descr = descr.replace(substring, tags[tagsUsed].value);
						break;
					case "PassiveSkillId":
					case "MoveId":
						descr = descr.replace(substring, Idx[idxValue]);
				}
				break;

			case "Digit":
				tagsUsed = getNextTagsUsedValue(tagsUsed, tags);
				lastQty = tags[tagsUsed].value;
				descr = descr.replace(substring, lastQty);
				break;
			...
			case "FR":
				switch(tagValue) {
					case "Qty":
						if(refValue != "") {
							lastQty = Idx[refValue];
						}

						replaceValue = singularValue;

						if(lastQty > 1) {
							replaceValue = pluralValue;
						}

						descr = descr.replace(substring, replaceValue);
						break;

					default:
						console.log(`------------------`);
						console.error("TAG VALUE NON GÉRÉE : " + tagValue);
						console.log(`Substring: ${substring}`);
						console.log(`tagType: ${tagType}`);
						console.log(`tagValue: ${tagValue}`);
						console.log(`idxValue: ${idxValue}`);
						console.log(`refValue: ${refValue}`);
						console.log(`singularValue: ${singularValue}`);
						console.log(`pluralValue: ${pluralValue}`);
				}
				break;
		}
	...
	while(descr.indexOf("[") !== -1) {
		...
		if(idxValue.length > 0) {
			if(tags[tagType] === undefined) {
				...
				Idx[idxValue] = tags[tagType][tagValue];
		...
		if(tagType === "FR") {
			...
			if(refValue.length > 0) {
				qtyRef = Idx[refValue][0];

				if(id == 10314) {
					console.log(Idx[refValue]);
				}
			}
...
async function init() {
	...
	passivesTable = document.getElementById("passivesTable");
